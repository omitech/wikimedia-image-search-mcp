...
import { buildWikimediaApiUrl, fetchWikimediaImages, parseWikimediaResponse, generateThumbnailComposite, } from "../wikimedia-api.js";
...
describe("wikimedia-api", () => {
    ...
    describe("parseWikimediaResponse", () => {
        ...
        it("should parse valid image results", () => {
            ...
            expect(result.images[0]?.thumbnailUrl).toBe(`https://example.com/${THUMBNAIL_SIZE}px-Test.jpg`);
    ...
    describe("fetchWikimediaImages", () => {
        it("should fetch and format search results with output files", async () => {
            ...
            writeFileSync(join(OUTPUT_DIR, "wikimediaSearchResults.json"), JSON.stringify(apiResponse, null, 2));
            // Parse response
            const searchResult = parseWikimediaResponse(apiResponse, params);
            // Save parsed search results
            writeFileSync(join(OUTPUT_DIR, "parsedSearchResults.json"), JSON.stringify(searchResult, null, 2));
            // format parsed results
            const formattedResults = formatSearchResults(searchResult);
            // Save formatted results
            writeFileSync(join(OUTPUT_DIR, "formattedSearchResults.txt"), formattedResults);
    ...
    describe("generateThumbnailComposite", () => {
        ...
        it("should generate composite from mock images", async () => {
            ...
            const fetchSpy = jest.spyOn(globalThis, "fetch").mockResolvedValue(new Response('<svg xmlns="http://www.w3.org/2000/svg" width="1" height="1"><rect width="1" height="1" fill="red"/></svg>'));
            try {
...
//# sourceMappingURL=wikimedia-api.test.js.map