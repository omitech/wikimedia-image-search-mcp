...
import { SearchImagesInputSchema } from "./schemas.js";
import { CHARACTER_LIMIT } from "./constants.js";
import yaml from "js-yaml";
import { fetchWikimediaImages, parseWikimediaResponse, generateThumbnailComposite, } from "./wikimedia-api.js";
...
export function formatSearchResults(result) {
    ...
    if (result.images.length === 0) {
        lines.push("No images found matching your query. Try different search query or change the license filter to 'all'.");
        return lines.join("\n");
    }
    ...
    lines.push(`\nShowing ${result.images.length} result${result.images.length !== 1 ? "s" : ""}:\n`);
 ...
 */
async function searchWikimediaImages(params) {
    try {
 ...
 */
server.registerTool("wikimedia_search_images", {
    title: "Search Wikimedia Commons Images",
    description: "Search for images on Wikimedia Commons with metadata including download URLs and optional thumbnail composite image for visual comparison. Use results to e.g. fetch full images that are relevant for your task.",
    ...
    try {
        ...
        if (textResponse.length > CHARACTER_LIMIT) {
            const truncatedResult = {
                ...searchResult,
                images: searchResult.images.slice(0, Math.floor(searchResult.images.length / 2)),
            };
        ...
        if (params.include_thumbnails && searchResult.images.length > 0) {
            const thumbnailComposite = await generateThumbnailComposite(searchResult.images);
