...
import { WIKIMEDIA_API_BASE, CC0_WIKIDATA_ID, MAX_RESULTS_LIMIT, THUMBNAIL_SIZE, MAX_IMAGES_IN_COMPOSITE, } from "./constants.js";
/**
 * Build Wikimedia Commons API URL for image search
 */
export function buildWikimediaApiUrl(params) {
 ...
 */
export async function fetchWikimediaImages(params) {
    const url = buildWikimediaApiUrl(params);
    const response = await fetch(url, {
        headers: {
            "User-Agent": "wikimedia-image-search-mcp/1.0.0 (https://github.com/yanexr/wikimedia-image-search-mcp)",
        },
    });
    if (!response.ok) {
...
function getAspectRatio(width, height) {
    ...
    let bestPair = [reducedWidth, reducedHeight];
 ...
 */
export function parseWikimediaResponse(apiResponse, params) {
    ...
    for (const page of slicedPages) {
        ...
        if (extmetadata?.UsageTerms?.value) {
            const truncated = truncateString(stripHtml(extmetadata.UsageTerms.value), 500);
            if (truncated)
                license.usageTerms = truncated;
        }
        ...
        if (extmetadata?.ObjectName?.value) {
            ...
            if (truncated)
                imageMetadata.caption = truncated;
        }
        ...
        if (extmetadata?.DateTimeOriginal?.value) {
            const truncated = truncateString(stripHtml(extmetadata.DateTimeOriginal.value), 500);
            if (truncated)
                imageMetadata.date = truncated;
        }
        if (extmetadata?.ImageDescription?.value) {
            const truncated = truncateString(stripHtml(extmetadata.ImageDescription.value), 500);
            if (truncated)
                imageMetadata.description = truncated;
        }
        if (extmetadata?.Credit?.value) {
            const truncated = truncateString(stripHtml(extmetadata.Credit.value), 500);
            if (truncated)
                imageMetadata.credit = truncated;
        }
        if (extmetadata?.Artist?.value) {
            const truncated = truncateString(stripHtml(extmetadata.Artist.value), 500);
            if (truncated)
                imageMetadata.artist = truncated;
        }
 ...
 */
export async function generateThumbnailComposite(images) {
    ...
    const imageBuffers = await Promise.all(limitedImages.map(async (img, index) => {
        try {
    ...
    const textOverlays = validImages.map(({ index }) => {
        const row = Math.floor(index / COLUMNS);
        const col = index % COLUMNS;
...
//# sourceMappingURL=wikimedia-api.js.map